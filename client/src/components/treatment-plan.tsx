...
const downloadLabels: Record<Language, string> = {
  English: "Download Plan",
  Telugu: "ప్లాన్ డౌన్‌లోడ్",
  Hindi: "प्लान डाउनलोड करें",
};
...
function downloadPlan(plan: string, language: Language) {
  const cleanText = cleanMarkdownToText(plan);
  const header = titles[language];
  const fullText = `${header}\n${"=".repeat(header.length)}\n\n${cleanText}\n\n---\nGenerated by KhetSathi - AI Crop Doctor`;

  const blob = new Blob([fullText], { type: "text/plain;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "KhetSathi-Treatment-Plan.txt";
    ...
    <Card className="p-4 sm:p-5 border-primary/20 max-h-[60vh] overflow-y-auto" data-testid="card-treatment-plan">
      <div className="flex items-center justify-between gap-2 mb-3 pb-2 border-b border-border">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => downloadPlan(plan, language)}
